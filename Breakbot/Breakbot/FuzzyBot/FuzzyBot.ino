...
// ============================================================
const int TURN_MS = 450;  // how long to turn toward chosen direction (ms)
...
void loop() {
  ...
  if (autoMode) {
    ...
    // ── Check the bump sensor ONLY in auto mode ─────────
    if (digitalRead(KNOCK_PIN) == HIGH) {
      stopMotors();

      if (!bumped) {
        Serial.println(">>> BUMP DETECTED (auto) – stopping <<<");
        bumped = true;
      }
      bumpClearTime = millis() + 200;   // keep flag for 200ms after last knock
    }

    // Clear the bump flag
    if (bumped && millis() > bumpClearTime && digitalRead(KNOCK_PIN) == LOW) {
      bumped = false;
      Serial.println("Bump cleared (auto)");
    }
